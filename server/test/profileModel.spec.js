...
describe('Profile model tests', function () {
  ...
  it('Should be able to update an already existing record', function (done) {
    Profile.where({ id: 1 }).fetch()
      ...
      .then(function () {
        return Profile.where({ id: 5 }).save({ first: 'Douglas', last: 'Cox' }, { method: 'update' });
      })
      .then(function () {
        return Profile.where({ id: 5 }).fetch();
      })
      .then(function (result) {
        expect(result.get('first')).to.equal('Douglas');
        expect(result.get('last')).to.equal('Cox');
        done();
      })
      .catch(function (err) {
        // If this expect statement is reached, there's an error.
